...
#'
#' @param alignment_id nucleotide-alignments id
...
#' }
#'
#' @return data from `nucleotide_alignments` endpoint
#' @export
#'
#' @rdname nucleotide_alignments
get_nucleotide_alignments <- function(alignment_id = NULL, ...) {
  endpoint <- "nucleotide-alignments"
  if (!is.null(alignment_id)) {
    endpoint <- glue::glue("nucleotide-alignments/{alignment_id}")
  }
