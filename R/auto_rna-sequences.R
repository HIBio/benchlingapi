#' List RNA sequences
#'
#' Chains of linear, single stranded RNA that support most capabilities and
#' attributes of DNA Sequences.
#'
#' @param rna_sequence_id rna-sequences id
...
#'    \item{name}{Name of an RNA Sequence. Restricts results to those with the specified name, alias, or entity registry ID.}
#'    \item{nameIncludes}{Name substring of an RNA Sequence. Restricts results to those with names, aliases, or entity registry IDs that include the provided substring.}
#'    \item{bases}{Full bases of the RNA sequence. Restricts results to those with the specified bases, case-insensitive, allowing for circular or reverse complement matches. Does not allow partial matching or loose matching via degenerate bases.}
#'    \item{folderId}{ID of a folder. Restricts results to those in the folder.}
#'    \item{mentionedIn}{Comma-separated list of entry IDs. Restricts results to RNA sequences mentioned in those entries.}
...
#' }
#'
#' @return data from `rna_sequences` endpoint
#' @export
#'
#' @rdname rna_sequences
get_rna_sequences <- function(rna_sequence_id = NULL, ...) {
  endpoint <- "rna-sequences"
  if (!is.null(rna_sequence_id)) {
    endpoint <- glue::glue("rna-sequences/{rna_sequence_id}")
  }
